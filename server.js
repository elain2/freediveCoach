...
function buildPrompt(rubric, mode) {
...

{
  ...
  "categories": [
    { "name": "항목명", "score": 점수(1~5), "note": "관찰 내용", "tip": "개선 팁" }
  ],
  "hook": "블로그용 후킹 카피 (1문장)"
}`;
}

async function callGemini(frames, prompt) {
  const parts = [];
...
app.post('/api/analyze', async (req, res) => {
  try {
    const { discipline, mode, frames } = req.body;
    ...
    console.log(`분석 시작: ${discipline}, ${frames.length}장`);
    ...
    res.status(500).json({ error: err.message || '분석 중 오류 발생' });
  }
});
